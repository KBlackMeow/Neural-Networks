...
def train(x,y,d):
    for i in range(len(x)):
        ...
        c=x[i]
        d.train(np.mat(c).T,np.mat(y[i]).T,1)
...
def detect(a):
    ...
    dat=svd(dat,28,28)
    plt.title("svd")
    plt.imshow(np.reshape(dat,[28,28]),cmap='gray')
    plt.show()
    dat= np.reshape(dat,28*28)
    dat=dat/np.average(dat)
    inp=np.mat(dat).T
    out=d.get_out(inp)
